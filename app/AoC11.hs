...
runRound mnks =
  foldl
    (\mnks i ->
       U.setList i (\(mnk, nb) -> (mnk {items = []}, nb + length (items mnk)))
...
runMonkey2 (Monkey2 id itms op (c, t, f)) mods =
  map
    (\is -> (if newLvl (is !! id) `mod` c == 0 then t else f
...
runRound2 mnks =
  foldl
    (\mnks i ->
       U.setList i (\(mnk, nb) -> (mnk {items2 = []}, nb + length (items2 mnk)))
       $ distribute2
