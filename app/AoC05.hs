...
parseStacks =
  ...
  -- ,"ZMP"]    ,"P"]
  . transpose
  -- for each line, isolate the letters representing the crates. if there are no
  -- crates it grabs a space ("    [D]" -> " D").
  . map (
    map (!! 1)
    . U.chunksOf 4
  )
...
parseInsts =
  map (
    ...
    -- represented by one digit. thus we reverse, grabe the stacks numbers with
    -- !!0 and !!1, drop them, re-reverse and read the arbitrarly long number of
    -- crates.
    . reverse                    -- "221" -> "122"
